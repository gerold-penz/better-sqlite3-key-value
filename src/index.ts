...
export class BetterSqlite3KeyValue {
    ...
    private getItemStatement: Statement<any, Omit<Record, "key">>
    private getAllItemsStatement: Statement
    private getItemsStartsWithStatement: Statement<any, Record>
    private getKeyStatement: Statement<any, Omit<Record, "key" | "value">>
    private getAllKeysStatement: Statement
    private getKeysStartsWithStatement: Statement<any, Omit<Record, "value">>
    private countExpiringStatement: Statement
    ...
    constructor(filename: string = ":memory:", options?: Options) {
        // Parse options
        const {ttlMs, ...otherOptions} = options ?? {}
        this.ttlMs = ttlMs
        const dbOptions: DbOptions = {...otherOptions}
        ...
        this.db.prepare("CREATE TABLE IF NOT EXISTS items (key TEXT PRIMARY KEY, value BLOB, expires INT)").run()
        this.db.prepare("CREATE UNIQUE INDEX IF NOT EXISTS ix_items_key ON items (key)").run()
        this.db.prepare("CREATE INDEX IF NOT EXISTS ix_items_expires ON items (expires)").run()
        ...
        this.countStatement = this.db.prepare("SELECT COUNT(*) AS count FROM items")

        this.getAllItemsStatement = this.db.prepare("SELECT key, value, expires FROM items")
        this.getItemStatement = this.db.prepare("SELECT value, expires FROM items WHERE key = $key")
        this.getItemsStartsWithStatement = this.db.prepare("SELECT key, value, expires FROM items WHERE key = $key OR key >= $gte AND key < $lt")

        this.getAllKeysStatement = this.db.prepare("SELECT key, expires FROM items")
        this.getKeyStatement = this.db.prepare("SELECT expires FROM items WHERE key = $key")
        this.getKeysStartsWithStatement = this.db.prepare("SELECT key, expires FROM items WHERE (key = $key OR key >= $gte AND key < $lt)")

        this.countExpiringStatement = this.db.prepare("SELECT COUNT(*) as count FROM items WHERE expires IS NOT NULL")
        this.deleteExpiringStatement = this.db.prepare(`
        DELETE FROM items
        WHERE key IN (
        ...
        if (startsWithOrKeys && typeof startsWithOrKeys === "string") {
            ...
            records = this.db.transaction(() => {
                return (startsWithOrKeys as string[]).map((key: string) => {
                    const record = this.getItemStatement.get({key})
                    return {...record, key} as Record
                })
            })()
        } else {
            // All items
            records = this.getAllItemsStatement.all() as Record[]
        }
    ...
    getKeys(startsWithOrKeys?: string | string[]): string[] | undefined {
        ...
        if (startsWithOrKeys && typeof startsWithOrKeys === "string") {
            ...
            // All items
            records = this.getAllKeysStatement.all() as Omit<Record, "value">[]
        }
        if (!records?.length) return
        const now = Date.now()
        const result: string[] = []
    ...
    getExpiringItemsCount() {
        return (this.countExpiringStatement.get() as {count: number}).count
    }
