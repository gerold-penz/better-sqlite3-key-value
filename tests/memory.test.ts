...
test("Set and get value", () => {
    const store: BetterSqlite3KeyValue = new BetterSqlite3KeyValue()
...
test("Default expiration ttlMs", async () => {
    const store: BetterSqlite3KeyValue = new BetterSqlite3KeyValue(undefined, {ttlMs: 30})
...
test("Caching with implicite TTL", async () => {
    const store: BetterSqlite3KeyValue = new BetterSqlite3KeyValue(undefined, {ttlMs: 40})
...
test("Store Map() values", () => {
    ...
    store.set(KEY_1, value)
    const result = store.get(KEY_1)
    expect(Array.from(result.entries())).toEqual(Array.from(value.entries()))
})
